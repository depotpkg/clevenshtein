 ...
 */
int levenshtein_n(const char* string1,
                  size_t len1,
                  const char* string2,
                  size_t len2,
                  int w,
  ...
  for (i = 0; i < len1; i++) {
    ...
    for (j = 0; j < len2; j++) {
      ...
      if (i > 0 && j > 0 && string1[i - 1] == string2[j] &&
        ...
        row2[j + 1] = row0[j - 1] + w;
      /* deletion */
      if (row2[j + 1] > row1[j + 1] + d) row2[j + 1] = row1[j + 1] + d;
      /* insertion */
      if (row2[j + 1] > row2[j] + a) row2[j + 1] = row2[j] + a;
    }
...
int levenshtein(
  ...
  return levenshtein_n(string1, len1, string2, len2, w, s, a, d);
}
